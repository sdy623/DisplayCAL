...
Name: {commonstartup}\%(AppName) Profile Loader; Filename: {reg:HKCU\Software\Zero Install,InstallLocation|{reg:HKLM\Software\Zero Install,InstallLocation}}\0install-win.exe; Parameters: "run --batch --no-wait --offline --command=run-apply-profiles %(URL)s0install/%(AppName).xml"; Tasks: calibrationloadinghandledbydisplaycal;
...
Filename: {reg:HKCU\Software\Zero Install,InstallLocation|{reg:HKLM\Software\Zero Install,InstallLocation}}\0install-win.exe; Parameters: "run --batch --command=set-calibration-loading %(URL)s0install/%(AppName).xml"; Description: {cm:LaunchProgram,%(AppName)}; Flags: RunAsCurrentUser; Tasks: calibrationloadinghandledbydisplaycal;
; Filename: {reg:HKCU\Software\Zero Install,InstallLocation|{reg:HKLM\Software\Zero Install,InstallLocation}}\0install-win.exe; Parameters: "integrate --machine --add=auto-start --batch %(URL)s0install/%(AppName).xml"; Description: {cm:LaunchProgram,%(AppName)}; Flags: RunAsCurrentUser; Tasks: calibrationloadinghandledbydisplaycal;
